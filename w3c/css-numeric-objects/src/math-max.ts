 ...
 * @internal
 */
function max(first: NumericValue, ...tail: NumericValue[]): NumericType {
  return tail.reduce(
    (currType: NumericType, item: NumericValue): NumericType => {
      const nextType = currType.max(item.type);

      // If type is failure, throw a TypeError.
      if (nextType == null) {
        throw new TypeError(
          "Failed to execute 'max' on 'NumericValue': Incompatible types",
        );
      }
...
export class MathMax extends MathValue {
  readonly values: NumericValue[];

  // The MathMax(...args) constructor must, when called, perform the
  // following steps:
  constructor(...args: Numberish[]) {
    ...
    if (!isNonEmptyArray(values)) {
      throw new SyntaxError(
        "Failed to construct `MathMax`: Arguments can't be empty",
      );
    }

    // 3. Let `type` be the result of maxing the types of all the items of args.
    // 4. Return a new `MathMax` whose values internal slot is set to args.
    super(max(...values), MathOperator.Max);
