 ...
 * @internal
 */
function min(first: NumericValue, ...tail: NumericValue[]): NumericType {
  return tail.reduce(
    (currType: NumericType, item: NumericValue): NumericType => {
      const nextType = currType.min(item.type);

      // If type is failure, throw a TypeError.
      if (nextType == null) {
        throw new TypeError(
          "Failed to execute 'min' on 'NumericValue': Incompatible types",
        );
      }
...
export class MathMin extends MathValue {
  readonly values: NumericValue[];

  // The MathMin(...args) constructor must, when called, perform the
  // following steps:
  constructor(...args: Numberish[]) {
    ...
    if (!isNonEmptyArray(values)) {
      throw new SyntaxError(
        "Failed to construct `MathMin`: Arguments can't be empty",
      );
    }

    // 3. Let `type` be the result of mining the types of all the items of args.
    // 4. Return a new `MathMin` whose values internal slot is set to args.
    super(min(...values), MathOperator.Min);
