...
export class MathInvert extends MathValue {
  value: NumericValue;

  // The MathInvert(arg) constructor must, when called, perform the
  // following steps:
  constructor(arg: Numberish) {
    ...
    // 2. Return a new `MathInvert` whose value internal slot is set to `arg`.
    super(value.type, MathOperator.Invert);
