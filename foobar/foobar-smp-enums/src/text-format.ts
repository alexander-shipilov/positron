...
/**
 * The {@link TextFormat} enumeration represents the text format flags.
 *
...
export enum TextFormat {
  ...
  /**
   * Draws without clipping. `GdiDrawText` is somewhat faster when
   * {@link TextFormat.NoClip} is used.
   */
  NoClip = 0x00000100,
   ...
   * If there are multiple lines of text, `GdiDrawText` uses the `width`
   * parameter and extends the base of the rectangle to bound the last line of
   * text. If the largest word is wider than the rectangle, the width is
   * expanded. If the text is less than the width of the rectangle, the width
   * is reduced. If there is only one line of text, `GdiDrawText` modifies the
   * right side of the rectangle so that it bounds the last character in the
   * line. In either case, `GdiDrawText` does not draw the text.
   */
  CalcRect = 0x00000400,

  /**
   * Turns off processing of prefix characters.
   * Normally, `GdiDrawText` interprets the mnemonic-prefix
