import type { AlbumArtType, SystemColour, VirtualKey } from "../enums";
 ...
 * @public
 */
export interface FbUtils {
  /**
   * The {@link FbUtils.Version} property represents a string corresponding to
   * the version.
   *
   * @remarks
  ...
  /**
   * The {@link FbUtils.CheckComponent} method checks the availability of
  ...
  /**
   * The {@link FbUtils.CheckFont} method checks if the font is installed.
   *
   * @remarks
  ...
  /**
   * The {@link FbUtils.ColourPicker} method spawns a windows popup dialog to
  ...
  /**
   * The {@link FbUtils.DetectCharset} method detects the codepage of the file.
   *
   * @remarks
  ...
  /**
   * The {@link FbUtils.EditTextFile} method edits a text file with the default
   * text editor.
   *
   * @remarks
  ...
  /**
   * The {@link FbUtils.FileExists} method checks if file exists.
   *
   * @param path - Path to file
   *
  ...
  /**
   * The {@link FbUtils.FormatDuration} method returns formatted duration.
   *
   * @example
   * ```ts
   *  console.log(
  ...
  /**
   * The {@link FbUtils.FormatFileSize} method returns formatted file size.
   *
   * @example
   * ```ts
  ...
  /**
   * The {@link FbUtils.GetAlbumArtAsync} method loads art image for the track
   * asynchronously.
   *
   *
   * @remarks
   ...
   *
   * @param windowId - {@link FbWindow.ID}.
   * @param handle -
   ...
   *   {@link FbCallbacks.on_get_album_art_done} callback. Default `false`.
   */
  GetAlbumArtAsync(
    windowId: number,
  ...
  /**
   * The {@link FbUtils.GetAlbumArtAsyncV2} method loads art image for the
   * track
   * asynchronously.
   *
   * @remarks
   * Returns a `Promise` object, which will be resolved when art loading is
   *   done.
   *
  ...
  /**
   * The {@link FbUtils.GetAlbumArtEmbedded} method loads embedded art image
   * for
   * the track.
   *
   * @remarks
   * Performance note: consider using {@link FbUtils.GetAlbumArtAsync} or
   *   {@link FbUtils.GetAlbumArtAsyncV2} if there are a lot of images to load.
   *
   * @example
   * ```ts
  ...
  /**
   * The {@link FbUtils.GetAlbumArtV2} method loads art image for the track.
   *
   * @remarks
   * Performance note: consider using {@link FbUtils.GetAlbumArtAsync} or
   *   {@link FbUtils.GetAlbumArtAsyncV2} if there are a lot of images to load.
  ...
  /**
   * The {@link FbUtils.GetFileSize} method returns file size in bytes.
   *
   * @param path - Path to file.
   */
  ...
  /**
   * The {@link FbUtils.GetPackageInfo} method returns an information about a
  ...
  /**
   * The {@link FbUtils.GetPackagePath} method returns a path to a package
   * directory with the specified id.
   *
   * @remarks
   * Throws an exception if package is not found.
   *
   * @param packageId -
   *
   * @deprecated Use {@link FbUtils.GetPackageInfo} instead.
   */
  GetPackagePath(packageId: string): string;

  /**
   * The {@link FbUtils.GetSysColour} method returns system colour for
   * specified `type`.
   *
   ...
   * ```
   *
   * @param type - One of {@link SystemColour}.
   *
   * @returns `0` if failed.
   */
  GetSysColour(type: SystemColour): number;

  /**
   * The {@link FbUtils.GetSystemMetrics} method returns the specified system
   * metrics.
  ...
  /**
   * The {@link FbUtils.Glob} method retrieves paths that match the supplied
   * pattern.
   *
   * @example
   * ```ts
   ...
   *
   * @param pattern -
   * @param excludeMask - Can be one or more of the {@link FileAttribute}.
   *   Default {@link FileAttribute.Directory}.
   * @param includeMask - Can be one or more of the {@link FileAttribute}.
   *   Default {@link FileAttribute.All}.
   */
  Glob(pattern: string, excludeMask?: number, includeMask?: number): string[];

  /**
   * The {@link FbUtils.InputBox} method displays a dialog with a message
  ...
  /**
   * The {@link FbUtils.IsDirectory} method checks if the specified `path` is a
   * directory.
   *
   * @param path -
   *
  ...
  /**
   * The {@link FbUtils.IsDirectory} method checks if the specified
   * `virtualKey` is pressed.
   *
  ...
  /**
   * todo: add doc comment and flags
   *
  ...
  /**
   * The {@link FbUtils.PathWildcardMatch} method checks if the supplied string
   * matches the pattern.
   *
   * @remarks
  ...
  /**
   * The {@link FbUtils.ReadINI} method reads value from INI file.
   *
   * @remarks
  ...
  /**
   * The {@link FbUtils.ReadTextFile} method reads file content.
   *
   * @remarks
  ...
  /**
   * The {@link FbUtils.ShowHtmlDialog} method displays a html dialog, rendered
   * by IE engine.
   *
   * @remarks
   * Utilizes the latest non-Edge IE that you have on your system. Dialog is
   *   modal (blocks input to the parent window while open).
   *
   ...
   *
   *  - Objects passed to `data` are limited to standard JavaScript objects: no
   *    extensions from `SpiderMonkey` panel (e.g. no {@link FbMetadbHandle}
   *    or {@link GdiBitmap}).
   *
  ...
  /**
   * The {@link FbUtils.SplitFilePath} method splits the specified `path` to
   * parts.
   *
   * @example
   * ```ts
   ...
   */
  SplitFilePath(path: string): [string, string, string];

  /**
   * The {@link FbUtils.WriteINI} method writes value to INI file.
   *
   * @example
   * ```ts
  ...
  /**
   * The {@link FbUtils.WriteTextFile} method writes content to the specified
   * file.
   *
   * @remarks
   * Note: the parent folder must already exist.
   *
   * Note: the file is written with UTF8 encoding.
   *
   * @example
   * ```ts
   *  // Default encoding
