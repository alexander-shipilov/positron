...
import { addEventListener, Point, Rect } from "positron-dom";
import { transform3d } from "utils/html";
...
export class MoveHandle extends Base {
    ...
    frame = new AnimationFrame(() => this.move(this.props));

    move(props) {
      const { ghost, target } = this;

      Object.assign((ghost || target).styles, transform3d(ghost || target, props));
    }
    ...
    moveTo(point) {
      ...
      if (constraint) {
        point = constraint(this).constrain(point);
      }

      if (direction !== MOVE_VERTICAL) {
        props.left = point.left;
      }
