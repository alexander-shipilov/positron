...
describe("TypedImmutableObject", () => {
    ...
    describe(".Type", () => {
        it("contains specified type", () => {
            expect(TypedImmutableObject.Type).toBe(ImmutableObject);
        });
    });

    describe(".of", () => {
        it("creates new typed class", () => {
            expect(TypedImmutableObject.of(Foo)).not.toBe(TypedImmutableObject);
            expect(TypedImmutableObject.of(Foo).Type).toBe(Foo);
        });

        it("should throw if the passed class does not implement ImmutableObject", () => {
            expect(() => TypedImmutableObject.of(() => void 0)).toThrow();
        });
    });
