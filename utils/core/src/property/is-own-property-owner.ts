...
/**
 * The {@link isOwnPropertyOwner} function returns `true` if the specified
 ...
 * @public
 */
export function isOwnPropertyOwner<TKey extends PropertyKey>(
  target: unknown,
  key: TKey,
): target is PropertyOwner<TKey> {
  return isNonNullable(target) && hasOwnProperty(target, key);
}
